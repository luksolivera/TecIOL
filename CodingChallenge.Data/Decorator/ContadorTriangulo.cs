...
{
    public class ContadorTriangulo : AbstractDecorator
    {
        public ContadorTriangulo(AbstractDecorator decorator) : base(decorator)
        {
        }
        ...
        {
            if (Cantidad > 0)
                str.Append($"{Cantidad} {idioma.TraducirForma(new Triangulo(), Cantidad)} | {idioma.Area()} {Area:#.##} | {idioma.Perimeter()} {Perimetro:#.##} <br/>");
