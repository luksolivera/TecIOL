...
{
    public class ContadorCirculo : AbstractDecorator
    {
        public ContadorCirculo(AbstractDecorator decorator) : base(decorator)
        {
        }
        ...
        {
            if (Cantidad > 0)
                str.Append( $"{Cantidad} {idioma.TraducirForma(new Circulo(), Cantidad)} | {idioma.Area()} {Area:#.##} | {idioma.Perimeter()} {Perimetro:#.##} <br/>");
